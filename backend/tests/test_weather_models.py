...
class TestValidateHrrrTimeRange:
    def test_accepts_recent_past_window(self) -> None:
        ...
        validate_hrrr_time_range(_utc(2026, 5, 10, 6), 6, now=now)

    def test_accepts_near_future_window(self) -> None:
        now = _utc(2026, 5, 10, 12)
        validate_hrrr_time_range(_utc(2026, 5, 10, 14), 6, now=now)

    def test_rejects_before_archive_start(self) -> None:
        with pytest.raises(WeatherTimeRangeError, match="archive start"):
            validate_hrrr_time_range(_utc(2014, 1, 1), 6)
...
class TestResolvePastcastCycles:
    def test_past_hours_use_analysis_fxx0(self) -> None:
        now = _utc(2026, 5, 10, 18)
        cycles = resolve_hrrr_cycles(_utc(2026, 5, 10, 6), 6, now=now)

        assert len(cycles) == 6
        for i, cycle in enumerate(cycles):
            expected_valid = _utc(2026, 5, 10, 6 + i)
    ...
    def test_pastcast_across_midnight(self) -> None:
        now = _utc(2026, 5, 11, 6)
        cycles = resolve_hrrr_cycles(_utc(2026, 5, 10, 22), 4, now=now)

        assert len(cycles) == 4
        assert cycles[0].valid_time == _utc(2026, 5, 10, 22)
        assert cycles[1].valid_time == _utc(2026, 5, 10, 23)
        assert cycles[2].valid_time == _utc(2026, 5, 11, 0)
        assert cycles[3].valid_time == _utc(2026, 5, 11, 1)
        for cycle in cycles:
            assert cycle.forecast_hour == 0
...
class TestResolveForecastCycles:
    def test_future_hours_use_latest_cycle(self) -> None:
        now = _utc(2026, 5, 10, 14)
        latest_cycle = _utc(2026, 5, 10, 12)  # 14 - 2h lag

        cycles = resolve_hrrr_cycles(_utc(2026, 5, 10, 16), 3, now=now)
    ...
    def test_future_valid_times_are_correct(self) -> None:
        ...
        assert cycles[0].valid_time == _utc(2026, 5, 10, 16)
        assert cycles[1].valid_time == _utc(2026, 5, 10, 17)
        assert cycles[2].valid_time == _utc(2026, 5, 10, 18)
    ...
    def test_far_future_falls_back_to_extended_cycle(self) -> None:
        ...
        now = _utc(2026, 5, 10, 14)
        latest_cycle = _utc(2026, 5, 10, 12)
...
class TestResolveMixedCycles:
    def test_mixed_window_transitions_from_past_to_future(self) -> None:
        now = _utc(2026, 5, 10, 14)
        latest_cycle = _utc(2026, 5, 10, 12)
        start = _utc(2026, 5, 10, 11)  # 1h before latest cycle
        ...
        assert cycles[0].analysis_time == _utc(2026, 5, 10, 11)
        assert cycles[1].forecast_hour == 0
        assert cycles[1].analysis_time == _utc(2026, 5, 10, 12)
...
class TestResolveCyclesEdgeCases:
    def test_single_hour_duration(self) -> None:
        now = _utc(2026, 5, 10, 18)
        cycles = resolve_hrrr_cycles(_utc(2026, 5, 10, 12), 1, now=now)
        assert len(cycles) == 1
        assert cycles[0].valid_time == _utc(2026, 5, 10, 12)
    ...
    def test_rejects_non_positive_duration(self) -> None:
        with pytest.raises(ValueError, match="positive"):
            resolve_hrrr_cycles(_utc(2026, 5, 10, 12), 0)

    def test_all_cycles_have_correct_valid_times(self) -> None:
        now = _utc(2026, 5, 10, 18)
        start = _utc(2026, 5, 10, 6)
    ...
    def test_analysis_time_plus_fxx_equals_valid_time(self) -> None:
        ...
        now = _utc(2026, 5, 10, 14)
        cycles = resolve_hrrr_cycles(_utc(2026, 5, 10, 10), 8, now=now)
...
class TestHrrrCycle:
    def test_frozen(self) -> None:
        cycle = HrrrCycle(
            analysis_time=_utc(2026, 5, 10, 12),
            forecast_hour=2,
            valid_time=_utc(2026, 5, 10, 14),
        )
        with pytest.raises(AttributeError):
            cycle.forecast_hour = 3  # type: ignore[misc]

    def test_equality(self) -> None:
        a = HrrrCycle(_utc(2026, 5, 10, 12), 2, _utc(2026, 5, 10, 14))
        b = HrrrCycle(_utc(2026, 5, 10, 12), 2, _utc(2026, 5, 10, 14))
        assert a == b
